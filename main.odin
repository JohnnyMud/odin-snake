...
draw_game :: proc(snake: []Point, food: Point) {
	// Move cursor to top-left, print score above the board, then redraw.
	fmt.print("\e[H")
	fmt.printf("Score: %d\e[K\n", score)
...
draw_centered_row :: proc(text: string) {
...
draw_game_over :: proc(final_score: int) {
	score_line := fmt.tprintf("Score: %d", final_score)
	...
	for y in 1 ..< WINDOW_HEIGHT - 1 {
		...
		case WINDOW_HEIGHT / 2 - 2:
			draw_centered_row("GAME OVER")
		case WINDOW_HEIGHT / 2:
			draw_centered_row(score_line)
		case WINDOW_HEIGHT / 2 + 2:
			draw_centered_row("Press R to replay")
		case:
			draw_centered_row("")
		}
	}
...
main :: proc() {
	...
	for {
		...
		if !wait_for_replay(stdin_fd, buffer[:]) {
			return
		}
	}
}

wait_for_replay :: proc(stdin_fd: posix.FD, buffer: []byte) -> bool {
	for {
		if poll_input(stdin_fd) {
			n, err := os.read(os.stdin, buffer)
			if err != nil || n <= 0 {
